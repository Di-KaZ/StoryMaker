...
export default class BlocStoryUpdate extends Vue{
  ...
  public getBlocStory(event: Event){
    ...
    const response = fetchAPI<BlocStoryDTO>("http://localhost:8080/blocstories/get/"+this.idBlocStory, METHODS.POST, this.idBlocStory);
    response.then(data =>{
  ...
  public updateBlocStory(event: Event){
    event.preventDefault();
    fetchAPI<BlocStoryDTO>("http://localhost:8080/blocstories/update", METHODS.POST, this.blocStory);
  }
...
<template>
  ...
  <form action="">
    ...
    <textarea v-model="blocStory.text" name="blocStory_text" id="blocStory_text" cols="50" rows="30" placeholder="La description de votre Histoire épique"></textarea><br>
