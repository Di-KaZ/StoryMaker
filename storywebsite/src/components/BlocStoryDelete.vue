<script lang="ts">
import { Vue } from 'vue-class-component';
import BlocStory from '../types/BlocStory';
import BaseStoryComponent, { METHODS } from '../utils/BaseStoryComponent';

export default class BlocStoryDelete extends BaseStoryComponent {
	blocStory: BlocStory = { id: 2, name: 'Continuer sa route', text: 'Lorem ipsum' };

	public deleteBlocStory(event: Event) {
		event.preventDefault();
		this.fetch<BlocStory>('blocstories/delete/' + this.blocStory.id, METHODS.DELETE, this.blocStory.id);
	}
}
</script>

<style scoped></style>

<template>
	<form action="">
		<p>{{ blocStory.name }}</p>

		<button @click="deleteBlocStory">Supprimer un bloc</button>
	</form>
</template>
